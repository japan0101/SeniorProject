...
public class GUIEnemyAgent : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        var debugEpisode = "Episode: " + enemyAgent.currentEpisode + " - Step: " + enemyAgent.StepCount;
        var debugReward = "Reward: " + enemyAgent.cumulativeReward.ToString(CultureInfo.InvariantCulture);

        // Select style based on reward value
        var rewardStyle = enemyAgent.cumulativeReward < 0f ? _negativeStyle : _positiveStyle;
