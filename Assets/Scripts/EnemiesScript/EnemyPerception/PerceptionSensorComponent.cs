...
{
    [Header("Sensor Settings")] public string sensorName = "SightSensor";

    [Header("Dependencies")] public SightDetector sightDetector;

    [Header("Normalization Settings")] [Tooltip("Matches the logic values: X=30, Y=5, Z=30")]
    public Vector3 observationRange = new(30f, 5f, 30f);
    ...
    public override ISensor[] CreateSensors()
    {
        if (sightDetector == null)
        {
            ...
            {
                Debug.LogError(
                    $"[CRITICAL] SightDetector missing on {name}. Sensor will be disabled to prevent crash.");
