using System;
using EnemiesScript;
using TMPro;
using UnityEngine;
using UnityEngine.Events;

public class PlayerHealth : MonoBehaviour
{
    [SerializeField] private PlayerStats _stats;

    [Header("References")] public TextMeshProUGUI HPDisplay;

    public UnityEvent<float> TrackOnEnemyDamageDealt;
    public UnityEvent<bool> TrackDeath;

    private Rigidbody rb;

    // Start is called once before the first execution of Update after the MonoBehaviour is created
    private void Awake()
    {
        _stats.ModifyHP(_stats.maxHP);
        rb = GetComponent<Rigidbody>();
        DisplayHP();
    }

    // Update is called once per frame
    private void Update()
    {
        if (_stats.currentHP <= 0) _stats.currentHP = _stats.maxHP;
    }

    private void OnCollisionEnter(Collision collision)
    {
        // Check if collision is with EnemyAttack layer
        //Debug.Log(collision.gameObject);

        if (collision.gameObject.tag == "EnemyAttacks")
        {
            var bullet = collision.gameObject.GetComponent<MainBullet>();
            if (bullet != null)
                //spawn hit FX then register damage
                TakeDamage(collision.contacts[0].point, bullet.damage);
            var attacks = collision.gameObject.GetComponent<EnemyAttack>();
            if (attacks != null) TakeDamage(collision.contacts[0].point, attacks.damage);
            //attacker.OnHitPlayer();
        }
    }

    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "EnemyAttacks")
        {
            var bullet = other.gameObject.GetComponent<MainBullet>();
            if (bullet != null)
                //spawn hit FX then register damage
                TakeDamage(other.ClosestPointOnBounds(other.transform.position), bullet.damage);
            var attacks =
                other.gameObject.GetComponent<EnemyAttack>(); // checks for attack component that hold the damages
            if (attacks != null)
            {
                TakeDamage(other.ClosestPointOnBounds(other.transform.position), attacks.damage); //deal damages
                //Direction from attacks to player
                var pureDirection = transform.position - other.transform.position;
                pureDirection.y = 0;
                var normDirection = pureDirection.normalized;
                TakeKnockback(normDirection, attacks.baseKnockbackForce);
            }
        }
    }

    public event Action OnPlayerHurt;
    public event Action OnPlayerDie;

    public void TakeDamage(Vector3 dmgPos, float damage)
    {
        //GetComponent<DamageTextSpawner>().SpawnDamageText(dmgPos, damage);
        TrackOnEnemyDamageDealt?.Invoke(damage);
        OnPlayerHurt?.Invoke();
        _stats.ModifyHP(-damage);
        if (IsDead())
        {
            TrackDeath?.Invoke(false);
            OnPlayerDie?.Invoke();
            //Destroy(gameObject);
        }

        // Debug.Log(_stats.currentHP);
        DisplayHP();
    }

    public void TakeKnockback(Vector3 direction, float force)
    {
        //knock player with force of the attack in a direction
        rb.AddForce(direction * force, ForceMode.Impulse);
    }

    public bool IsDead()
    {
        //for use in enemy attaack detection
        return _stats.currentHP <= 0;
    }

    private void DisplayHP()
    {
        HPDisplay.text = "Health: " + _stats.currentHP + " / " + _stats.maxHP;
    }
}