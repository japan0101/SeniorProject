...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("References")] public EnergyManager energyManager;
    ...
    [Header("GroundCheck")] public float playerHeight;
    ...
    [Header("Movement")] private float speed;
    ...
    private void MyInput()
    {
        ...
        if (Input.GetKey(jumpKey) && readyToJump && grounded && energyManager.consumeEnergy(_stats.jumpCost))
        {
            ...
            if (resetJumpRoutine != null) StopCoroutine(resetJumpRoutine);
        ...
        if (Input.GetKey(dashKey) && readyToDash && energyManager.consumeEnergy(_stats.dashCost))
        {
            ...
            // If there's an existing coroutine, stop it first
            if (resetDashRoutine != null) StopCoroutine(resetDashRoutine);
        ...
        {
            if (!_stats.States.Contains(PlayerStates.Sprinting)) _stats.States.Add(PlayerStates.Sprinting);
        }
        else
        {
    ...
    private void MovePlayer()
    {
        ...
        else if (_stats.States.Contains(PlayerStates.Sprinting) && energyManager.consumeEnergy(_stats.sprintCost))
    ...
    private void SpeedControl()
    {
        ...
        if (_stats.States.Contains(PlayerStates.Dashing)) //Dashing
            speed = _stats.dashForce;
        else if (_stats.States.Contains(PlayerStates.Sprinting)) //Sprinting
            speed = _stats.sprintSpeed;
        else //Normal Walking
            speed = _stats.moveSpeed;
